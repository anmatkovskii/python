...
def wallet(message):
    ...
    if str(user_id) not in id_list:
        with open("D:\Programming\hw_itstep\TG Bots\shop_bot\wallet.json", "r") as file:
            data = json.load(file)
        ...
        with open("D:\Programming\hw_itstep\TG Bots\shop_bot\wallet.json", "w") as file:
            json.dump(data, file, indent=2)
            json.dump(user, file, indent=2)
    with open("D:\Programming\hw_itstep\TG Bots\shop_bot\wallet.json", "r") as file:
        data = json.load(file)
        next_step = bot.send_message(message.chat.id, f"Your current ballance: {data['Ballance']}",
...
def shop(message):
    with open("D:\Programming\hw_itstep\TG Bots\shop_bot\shop.txt", "r") as file:
        data = file.read()
